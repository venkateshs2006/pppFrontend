-- إضافة بيانات تجريبية مصححة للمنصة
...
-- إضافة المهام التجريبية (باستخدام القيم الصحيحة للـ enum)
...
(gen_random_uuid(), 'تطوير سياسة الإجازات والغياب', 'وضع سياسة شاملة لإدارة الإجازات وحالات الغياب', 'not_started', 'medium', '2024-12-15', 20, 0, (SELECT id FROM projects_2025_10_30_10_19 WHERE name LIKE '%الموارد البشرية%' LIMIT 1)),
...
(gen_random_uuid(), 'إعداد دليل الإجراءات المحاسبية', 'تطوير دليل مفصل للإجراءات المحاسبية والمالية', 'not_started', 'high', '2024-12-31', 45, 0, (SELECT id FROM projects_2025_10_30_10_19 WHERE name LIKE '%السياسات المالية%' LIMIT 1)),
...
(gen_random_uuid(), 'تطوير سياسات الأمن السيبراني', 'وضع سياسات شاملة لحماية أنظمة المعلومات', 'review', 'critical', '2024-11-05', 80, 75, (SELECT id FROM projects_2025_10_30_10_19 WHERE name LIKE '%الأمن السيبراني%' LIMIT 1)),
(gen_random_uuid(), 'إعداد دليل إجراءات الطوارئ', 'تطوير دليل شامل لإجراءات التعامل مع حالات الطوارئ', 'blocked', 'high', '2024-12-01', 30, 5, (SELECT id FROM projects_2025_10_30_10_19 WHERE name LIKE '%العمليات التشغيلية%' LIMIT 1))
ON CONFLICT (id) DO NOTHING;

-- إضافة المخرجات التجريبية (شجرة المخرجات)
WITH guide_id AS (
  INSERT INTO deliverables_2025_10_30_10_19 (id, title, description, type, status, version, project_id, parent_id, order_index) 
  VALUES (gen_random_uuid(), 'دليل سياسات الموارد البشرية', 'الدليل الرئيسي لجميع سياسات وإجراءات الموارد البشرية', 'guide', 'draft', '1.0', (SELECT id FROM projects_2025_10_30_10_19 WHERE name LIKE '%الموارد البشرية%' LIMIT 1), NULL, 1)
  ON CONFLICT (id) DO NOTHING
  RETURNING id
),
topic_id AS (
  INSERT INTO deliverables_2025_10_30_10_19 (id, title, description, type, status, version, project_id, parent_id, order_index) 
  VALUES (gen_random_uuid(), 'سياسات التوظيف والاختيار', 'مجموعة السياسات المتعلقة بعمليات التوظيف والاختيار', 'topic', 'review', '1.0', (SELECT id FROM projects_2025_10_30_10_19 WHERE name LIKE '%الموارد البشرية%' LIMIT 1), (SELECT id FROM guide_id), 1)
  ON CONFLICT (id) DO NOTHING
  RETURNING id
)
...
(gen_random_uuid(), 'سياسة التوظيف الداخلي', 'سياسة تنظم عمليات التوظيف الداخلي والترقيات', 'policy', 'approved', '1.2', (SELECT id FROM projects_2025_10_30_10_19 WHERE name LIKE '%الموارد البشرية%' LIMIT 1), (SELECT id FROM topic_id), 1),
(gen_random_uuid(), 'إجراءات المقابلات الشخصية', 'إجراءات مفصلة لتنفيذ المقابلات الشخصية', 'procedure', 'draft', '1.0', (SELECT id FROM projects_2025_10_30_10_19 WHERE name LIKE '%الموارد البشرية%' LIMIT 1), (SELECT id FROM topic_id), 2),
(gen_random_uuid(), 'قالب تقييم المرشحين', 'قالب موحد لتقييم المرشحين للوظائف', 'template', 'published', '2.0', (SELECT id FROM projects_2025_10_30_10_19 WHERE name LIKE '%الموارد البشرية%' LIMIT 1), (SELECT id FROM topic_id), 3),
...
(gen_random_uuid(), 'دليل الحوكمة المؤسسية', 'الدليل الرئيسي للحوكمة والامتثال المؤسسي', 'guide', 'draft', '0.5', (SELECT id FROM projects_2025_10_30_10_19 WHERE name LIKE '%الحوكمة%' LIMIT 1), NULL, 1)
ON CONFLICT (id) DO NOTHING;
...
(gen_random_uuid(), 'طلب مراجعة عاجلة لسياسة الأمان', 'نحتاج مراجعة عاجلة لسياسة الأمان بعد الحادث الأخير', 'open', 'urgent', 'مراجعة عاجلة', '2024-11-02', (SELECT id FROM projects_2025_10_30_10_19 WHERE name LIKE '%الأمن السيبراني%' LIMIT 1))
ON CONFLICT (id) DO NOTHING;
...
(gen_random_uuid(), 'ticket', (SELECT id FROM tickets_2025_10_30_10_19 WHERE title LIKE '%مراجعة عاجلة%' LIMIT 1), '2024-11-02 23:59:59', 'overdue', 12, NULL)
ON CONFLICT (id) DO NOTHING;
